...
test("TC-021_求職者履歴（操作証跡 No=21）", async ({ page }, testInfo) => {
  const prepared = await prepareEvidence({ rowIndex: 22 });
  ...
  await expect(page.locator("table").nth(1).getByText("終了")).toBeVisible();

  // 証跡: 22行目の「証跡ファイル」を参照して保存＋Excel追記
