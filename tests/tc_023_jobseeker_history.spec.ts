...
test("TC-023_求職者履歴（操作証跡 No=23）", async ({ page }, testInfo) => {
  const prepared = await prepareEvidence({ rowIndex: 24 });
  ...
  const sortLink = page.getByRole("link", { name: /作成日時/ });
  const before = await sortLink.textContent();
  await sortLink.click();
  await expect(page).toHaveURL(/sort=createdAt_/);
  const after = await page.getByRole("link", { name: /作成日時/ }).textContent();
  expect(after).not.toBe(before);

  // 証跡: 24行目の「証跡ファイル」を参照して保存＋Excel追記
